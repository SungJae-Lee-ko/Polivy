...
## DC 신청 양식의 한글 섹션명 (최우선 매핑 대상!)
- "허가사항" - 약물의 허가된 적응증, 용법용량, 투여기간
- "신청사유" - 이 약을 병원에 도입해야 하는 이유와 필요성
- "효능" - 임상시험 결과, 치료효과, 유효성 데이터
- "안전성" - 부작용, 이상반응, 주의사항, 금기
- "비용" - 약가, 경제성, 비용대비 효과
- "기타" - 위 5가지에 해당하지 않는 추가 정보 (장점, 편리성, 모니터링 등)
...
## 매핑 규칙 (매우 중요!)
1. 셀 라벨에 한글 섹션명("허가사항", "신청사유", "효능", "안전성", "비용", "기타")이 포함되면, 그것과 정확히 일치하는 키를 선택하세요 (최우선)
2. 예: 라벨에 "허가사항"이 있으면 → {{허가사항}} 선택
3. 라벨에 명시적 섹션명이 없으면 → 내용의 의미에 따라 가장 가까운 한글 섹션명 선택
4. 매우 불명확한 경우에만 "unknown"을 사용하세요

## JSON 응답 형식
```json
{{
  "태그_매핑": [
    {{"cell_id": "T0R1C2", "placeholder_key": "효능", "확신도": "높음"}},
    {{"cell_id": "T0R1C3", "placeholder_key": "안전성", "확신도": "중간"}},
    ...
  ]
}}
```
