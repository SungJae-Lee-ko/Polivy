...
## 매핑 우선순위 (좌측 섹션명이 명시된 경우 참고)
1. 셀 라벨이 "허가사항", "허가", "적응증", "용법용량" 포함 → "indication_dosage" 선택
2. 셀 라벨이 "신청사유", "도입사유", "배경" 포함 → "application_reason" 선택
3. 셀 라벨이 "효능", "유효성", "임상", "효과" 포함 → "efficacy" 선택
4. 셀 라벨이 "안전", "안전성", "부작용", "이상반응" 포함 → "safety" 선택
5. 셀 라벨이 "비용", "경제성", "약가" 포함 → "cost_effectiveness" 선택
6. 셀 라벨이 "기타", "고려사항", "주의점" 포함 → "other_considerations" 선택
7. 위 패턴에 일치하지 않으면 "unknown" 설정

## 응답 규칙
- 반드시 아래 JSON 형식으로만 응답하세요.
- placeholder_key는 반드시 위 목록에 있는 키만 사용하세요.
- 적합한 키가 없으면 "unknown"으로 설정하세요.
- 확신도: "높음"(명확히 일치) / "중간"(대략 일치) / "낮음"(불확실)
